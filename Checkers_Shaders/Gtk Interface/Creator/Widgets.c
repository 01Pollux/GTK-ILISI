    ...
    GtkAccelGroup* AccelGroup)
{
    ...
    else if (!strcmp(ElementName, "Shader"))
    {
        Shader_CreateWithXml(IdList, Element, Tree);
    }
...
void WidgetContainer_PrivateElements(WidgetIdList* IdList[26], WidgetContainer* Tree, XmlElement* Element)
{
    ...
    while (Element)
    {
        ...
        if (!strcmp(Name, "Style"))
        {
            ...
            if (Attr = XML_FindAttribute(Element, "Path"))
            {
                Widget_AddStyleFile(Tree->Widget, XML_GetAttributeValue(Attr));
            }
 ...
 * Fonction: L'ajout de style a un widget
 */
void Widget_AddStyleFile(GtkWidget* Widget, const gchar* CssPath)
{
    // Declaration d'une variable de CSS provider
    GtkCssProvider* Provider = gtk_css_provider_new();
