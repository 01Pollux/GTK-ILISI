...
static XmlElement* XML_AddToRoot(XmlElement* Root, char* Name, XmlElement* Children, XmlAttribute* Attributes)
{
    XmlElement* Element = (XmlElement*)calloc(1, sizeof(XmlElement));
    Malloc_Valid(Element, "XML_AddToRoot\n");
...
static XmlAttribute* XML_AddAttribute(XmlAttribute* Attribute, char* Key, char* Value)
{
    ...
    Malloc_Valid(Element, "XML_AddAttribute\n");
...
static char* XML_ProcessChildrens(char* Str, XmlElement** RootElement, char* RootName)
{
    ...
    while (*Str != '\0')
    {
        if (*Str == '<')
        {
            ...
            Str = XML_ProcessChildrens(Str, &Children, Token);
            Element = XML_AddToRoot(Element, Token, Children, Attributes);
        }
        else
        {
            ++Str;
...
XmlDocument* XML_OpenFile(const char* FileName)
{
    ...
    Malloc_Valid(Document, "XML_OpenFile\n");
    ...
    while (Str < StrEnd)
    {
        if (*Str == '<')
        {
            ...
            XmlElement* Children = NULL;
            Str = XML_ProcessString(Str, &InsideString, &BreakLoop);
            Str = XML_ProcessChildrens(Str + 1, &Children, Token);
            RootElement = XML_AddToRoot(RootElement, Token, Children, Attributes);
...
{
    return Document->Elements ? Document->Elements->Children : NULL;
}
