...
{
    ShaderData Data;
    ShaderData_Init(&Data);

    XmlAttribute* Attr = XML_GetAttributes(Element);
    const char* AttrName;

    gboolean NotSquare = FALSE;

    while (Attr)
    {
        AttrName = XML_GetAttributeName(Attr);
        if (strcmp(AttrName, "VS") == 0)
        {
            Data.VertexShader = (char*)XML_GetAttributeValue(Attr);
        }
        else if (strcmp(AttrName, "PS") == 0)
        {
            Data.PixelShader = (char*)XML_GetAttributeValue(Attr);
        }
        else if (strcmp(AttrName, "Vertices") == 0)
        {
            const char* AttrVal = XML_GetAttributeValue(Attr);
            // split like ({X,Y,Z}, {X, Y, Z}, ...)

            char* Token = strtok((char*)AttrVal, "({,})");
            int i = 0;
            while (Token)
            {
                Data.Vertices = (VS_Input*)realloc(Data.Vertices, sizeof(VS_Input) * (Data.VerticesCount++));

                Data.Vertices[i].Position[0] = (float)atof(Token);
                Token = strtok(NULL, "({,})");

                Data.Vertices[i].Position[1] = (float)atof(Token);
                Token = strtok(NULL, "({,})");

                Data.Vertices[i].Position[2] = (float)atof(Token);
                Token = strtok(NULL, "({,})");
        ...
        else if (strcmp(AttrName, "Indices") == 0)
        {
            ...
            while (Token)
            {
                Data.Indices = (int*)realloc(Data.Indices, sizeof(int) * (Data.IndicesCount++));
                Data.Indices[i++] = atoi(Token);
                Token = strtok(NULL, "({,})");
            }
            NotSquare = TRUE;
        }
        else if (!strcmp(AttrName, "UpdateSpeed"))
        {
            Data.UpdateSpeed = XML_GetAttributeValueFloat(Attr);
        }
    ...
    Parent->Widget = Shader_Create(&Data, GTK_WINDOW(Window->Widget));
}
