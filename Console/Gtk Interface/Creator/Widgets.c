...
void WidgetContainer_Free(WidgetContainer* Tree)
{
	...
	if (Tree)
	{
		...
		//des fils:
		WidgetContainer_Free(Tree->Childrens);
		//des suivants:
	...
	WidgetContainer* Parent)
{
	...
	*/
	...
	{
		Window_CreateWithXml(IdList, Element, Tree);
	}
...
/*
* Prototype: void WidgetContainer_ReadChildrens(WidgetIdList* IdList[26],
	XmlElement* Element,
	WidgetContainer* Parent);
...
*/
void WidgetContainer_ReadChildrens(WidgetIdList* IdList[26],
	XmlElement* Element,
	WidgetContainer* Parent)
{
	//Creation et initialisation de structure des fils
	WidgetContainer* Childrens = NULL, * Tmp;
	//Importation des elements
	XmlElement* SubItem = XML_GetElementChildrens(Element);
	...
	while (SubItem)
	{
		...
		if (Tmp)
		{
			...
			Childrens = WidgetContainer_AddContainer(Childrens, Tmp);
		}
		//On passe au suivant
	...
	if (Childrens)
	{
		...
		Parent->Childrens = WidgetContainer_AddContainer(Parent->Childrens, Childrens);
	}
}
...
WidgetLayout* WidgetLayout_ReadXml(XmlDocument* Document)
{
	...
	while (Element)
	{
		...
		Window = WidgetContainer_Read(Layout->IdList, Element, NULL);
...
void WidgetContainer_PrivateElements(WidgetIdList* IdList[26], WidgetContainer* Tree, XmlElement* Element)
{
	...
	Element = XML_GetElementChildrens(Element);
	...
	XmlAttribute* Attr;
	Element = XML_GetElementChildrens(Private);
	...
	while (Element)
	{
		...
		if (!strcmp(Name, "Style"))
		{
			...
			WidgetContainer_AddStyles(Tree->Widget, Element);
		}
